...
const EdukasiTab = () => {
  ...
  return (
    <div className="tab-section">
      <div>
        <h2 style={{ fontSize: "24px", fontWeight: 700, color: "#0f172a", letterSpacing: "-0.025em" }}>Edukasi</h2>
        <p style={{ fontSize: "14px", color: "#64748b" }}>Mari belajar tentang kualitas udara dan cara menjaganya.</p>
      </div>
      ...
      <div className="grid-4">
        {eduCards.map((card, i) => (
          <div key={i} className="edu-card">
            <div>
              <div className="edu-card-icon" style={{ background: card.iconBg, color: card.iconColor }}>
                {card.icon}
              </div>
              <h4 style={{ fontWeight: 700, color: "#1e293b", fontSize: "14px", marginBottom: "6px" }}>{card.title}</h4>
              <p style={{ fontSize: "12px", color: "#64748b", lineHeight: 1.6 }}>{card.desc}</p>
            </div>
            <button className="edu-card-btn" onClick={() => openModal(card.title, card.modalContent)}>
              Baca Selengkapnya
            </button>
          </div>
        ))}
      ...
      <EdukasiModal isOpen={modal.isOpen} title={modal.title} content={modal.content} onClose={closeModal} />
    </div>
  );
};
